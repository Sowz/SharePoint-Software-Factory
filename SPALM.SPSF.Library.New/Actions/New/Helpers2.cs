    ...
    public class Helpers2
    {
        ...
        public static ProjectItems GetDeploymentPath(DTE dte, Project project, SPFileType sPFileType, string deploymentPath)
        {
            ...
            if (IsSharePointVSTemplate(dte, project))
            {
                ...
                mappedfolder = GetMappedFolder(dte, project, sPFileType);
            }
            else
            {
        ...
        public static bool BuildProject(Project project)
        {
            ...
            ISharePointProjectService projectService = GetSharePointProjectService(project.DTE);
        ...
        public static bool IsSharePointVSTemplate(DTE dte, Project selectedProject)
        {
            ...
            try
            {
                ...
                ISharePointProjectService projectService = GetSharePointProjectService(selectedProject.DTE);
        ...
        private static void AddVSMappedFolderToVSPackage(DTE dte, Project project, ProjectItem newMappedFolder)
        {
            ...
            try
            {
                IMappedFolder sharePointMappedFolder = projectService.Convert<ProjectItem, IMappedFolder>(newMappedFolder);
                if (!sharePointProject.Package.ProjectItems.Contains(sharePointMappedFolder))
                {
                    ...
                    ProjectItem packageFile = projectService.Convert<ISharePointProjectMember, ProjectItem>(sharePointProject.Package.PackageFile);
        ...
        internal static void AddVSElementToVSFeature(DTE dte, Project project, ProjectItem elementFolder, string evaluatedParentFeatureName)
        {
            ...
            ISharePointProjectFeature sharePointFeature = projectService.Convert<ProjectItem, ISharePointProjectFeature>(featureItem);
            ISharePointProjectItem sharePointItem = projectService.Convert<ProjectItem, ISharePointProjectItem>(elementFolder);
            ...
            ProjectItem featureFile = projectService.Convert<ISharePointProjectMember, ProjectItem>(sharePointFeature.FeatureFile);
        ...
        internal static ProjectItem AddFileToElementManifest(DTE dte, Project project, ProjectItem ParentElementFolder, SPFileType sPFileType, string evaluatedTargetFolder, string fullSourceFileName, string evaluatedTargetFileName, bool overwrite, bool open)
        {
            if (IsSharePointVSTemplate(dte, project))
            {
                ...
                try
                {
                  ...
                  ISharePointProjectItem parentSharePointItem = projectService.Convert<ProjectItem, ISharePointProjectItem>(ParentElementFolder);
          ...
          //if project has been reloaded the parent item is not null, but not valid anymore
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        //can we convert the feature to a sharepointfeature and check the scope???
                        ISharePointProjectService projectService = GetSharePointProjectService(service);
                        ISharePointProject sharePointProject = projectService.Convert<Project, ISharePointProject>(_CurrentProject);
                        sharePointProject.Synchronize();
                    }
                    catch { }
                }
        ...
        internal static ProjectItem AddVSFeatureToVSPackage(DTE dte, Project project, string finalFeatureName)
        {
            ...
            try
            {
                ...
                if (!sharePointPackage.Features.Contains(sharePointFeature))
                {
                    ...
                    try
                    {
                      ProjectItem packageFile = projectService.Convert<ISharePointProjectMember, ProjectItem>(sharePointPackage.PackageFile);
                      packageFile.Save();
                    }
                    catch { }
                }
        ...
        */
        ...
        internal static void CopyDeploymentPath(ProjectItem SourceElementFile, ProjectItem TargetElementFile)
        {
            try
            {
                ...
                ISharePointProject sharePointProject = projectService.Convert<Project, ISharePointProject>(SourceElementFile.ContainingProject);
                sharePointProject.Synchronize();

                ISharePointProjectItemFile sourceItem = projectService.Convert<ProjectItem, ISharePointProjectItemFile>(SourceElementFile);
                ISharePointProjectItemFile targetItem = projectService.Convert<ProjectItem, ISharePointProjectItemFile>(TargetElementFile);
        ...
        {
          if (IsSharePointVSTemplate(item.DTE, item.ContainingProject))
          {
            try
            {
              ...
              ISharePointProjectItem sharePointItem = projectService.Convert<ProjectItem, ISharePointProjectItem>(item);
              if(sharePointItem == null)
              {
                ...
                {
                  sharePointItem = projectService.Convert<ProjectItem, ISharePointProjectItem>(item.Collection.Parent as ProjectItem);
                }
              }
