    ...
    public static class Helpers
    {
        ...
        public static string GetDiscoPath()
        {
            ...
            */
            ...
            string[] programmFilesFolders = new string[] { GetProgramsFolder32(), GetProgramsFolder() };
        ...
        public static ProjectItem AddFile(ProjectItem parentFolder, string sourceFilePath)
        {
            ...
            try
            {
                ProjectItem existingFile = GetProjectItemByName(parentFolder.ProjectItems, filename);
            ...
            return AddFromFileCopy(parentFolder, sourceFilePath);
        }
        ...
        {
            string projectpath = GetFullPathOfProjectItem(project);
            ...
            if (manifest != null)
            {
                string manifestpath = GetFullPathOfProjectItem(manifest);
        ...
        private static string GetOutputFolder(Project project)
        {
            ...
            try
            {
                return Path.Combine(GetFullPathOfProjectItem(project), project.Properties.Item("TargetZone").Value.ToString());
            }
            catch (Exception)
            {
            }
        ...
        public static void RunProcessAsync(DTE dte, string command, string parameters, string WorkingDirectory)
        {
            try
            {
                LogMessage(dte, dte, "Running command \"" + command + "\" " + parameters);
                LogMessage(dte, dte, "Please wait");
                ...
                LogMessage(dte, dte, p.StandardOutput.ReadToEnd());

                if (p.ExitCode != 0)
                {
                    LogMessage(dte, dte, "Failure");
                }
                else
                {
                    LogMessage(dte, dte, "Finished successfully");
                }              
                
                ShowProgress(dte, "Finished...", 100);
            }
            catch (Exception ex)
            {
                LogMessage(dte,dte, "Error: " + ex.ToString());
            }
        }
        ...
        public static bool IsCustomizationProject(Project project)
        {
            ...
            try
            {
                string manifestXml = Path.Combine(GetProjectFolder(project), "manifest.xml");
            ...
            try
            {
                ...
                ProjectItem file12 = GetProjectItemByName(project.ProjectItems, "12");
                if (file12 != null)
                {
                    return true;
                }
                ProjectItem file14 = GetProjectItemByName(project.ProjectItems, "14");
                if (file14 != null)
                {
                    return true;
                }
                ProjectItem file14b = GetProjectItemByName(project.ProjectItems, "SharePointRoot");
                if (file14b != null)
                {
                    return true;
                }                
                ProjectItem file15 = GetProjectItemByName(project.ProjectItems, "15");
                if (file15 != null)
                {
                    return true;
                }
        ...
        internal static ProjectItem GetFeatureXML(Project _CurrentProject, string ParentFeatureName)
        {
            ...
            if (Helpers2.IsSharePointVSTemplate(_CurrentProject.DTE, _CurrentProject))
            {
                if (featureFolder != null)
                {
                    ...
                    featureXMLFile = GetProjectItemByName(featureFolder.ProjectItems, ParentFeatureName + ".feature");
                }
            }
            else
            {
                ...
                {
                    //is in the feature already the contenttypes.xml?
                    featureXMLFile = GetProjectItemByName(featureFolder.ProjectItems, "feature.xml");
                }
            }
        ...
        {
            string path = GetFullPathOfProjectItem(featureXMLFile);
        ...
        internal static string GetAssemblyName(Project p, string snPath)
        {
            try
            {
                ...
                Process snProcess2 = Process.Start(startInfo2);
        ...
        */
        ...
        {
            string filename = GetFullPathOfProjectItem(item);
        ...
        internal static ProjectItem GetFeatureFolder(Project selectedProject, bool createIfNotExists)
        {
            try
            {
                ...
                ISharePointProject sharePointProject = projectService.Convert<Project, ISharePointProject>(selectedProject);
                if (sharePointProject != null)
                {
            ...
            try
            {
                featuresFolder = GetFolder(GetFolder(GetFolder(selectedProject, GetSharePointHive(selectedProject.DTE), createIfNotExists), "Template", createIfNotExists), "Features", createIfNotExists);
            }
            catch (Exception)
            {
            }
            ...
            try
            {
                ...
                {
                    //try "SharePointRoot"
                    featuresFolder = GetFolder(GetFolder(GetFolder(selectedProject, "SharePointRoot", createIfNotExists), "Template", createIfNotExists), "Features", createIfNotExists);
                }
            }
            catch (Exception)
            {
        ...
        {
            LogMessage(dte, dte, "Starting import of WSP file " + _WSPFilename);
            ...
            LogMessage(dte, dte, "Import finished");
        ...
        static public void CopyFolder(DTE dte, string sourceFolder, string destFolder, ProjectItems parentProjectItems)
        {
            ...
            foreach (string file in files)
            {
                ...
                if (File.Exists(dest) && !dte.SuppressUI)
                {
                    ...
                    {
                        LogMessage(dte, dte, "Warning: File " + dest + " replaced.");
                ...
                { //add file to project items
                    AddFromFile(parentProjectItems, dest);
                }
                catch (Exception)
        ...
        internal static ProjectItem IncludeFolderInProject(string path, ProjectItems projectItems, string subfoldername)
        {
            ...
            try
            {
                ...
                ProjectItem dummyfile = AddFromFile(projectItems, dummyfilepath);
                //return the created folder
                res = GetProjectItemByName(projectItems, subfoldername);
        ...
        {
            return GetFolder(GetFullPathOfProjectItem(_CurrentProject), _CurrentProject.ProjectItems, folderName, createIfNotExists);
        }

        internal static ProjectItem GetFolder(ProjectItem _ProjectItem, string folderName, bool createIfNotExists)
        {
            return GetFolder(GetFullPathOfProjectItem(_ProjectItem), _ProjectItem.ProjectItems, folderName, createIfNotExists);
        }
        ...
        internal static void AddElementsFileToFeature(DTE dte, ProjectItem featurefolder, string resfilename)
        {
            ...
            string featurepath = GetFullPathOfProjectItem(featureXMLFile);

            LogMessage(dte, dte, "Adding elements file to feature " + featurepath);
            ...
            EnsureCheckout(dte, featurepath);

            XmlWriter xw2 = XmlWriter.Create(featurepath, GetXmlWriterSettings(featurepath));
            featuredoc.Save(xw2);
            xw2.Flush();
            xw2.Close();
        }
        ...
        internal static bool ContainsCustomizationProject(SolutionFolder f)
        {
            foreach (ProjectItem pi in f.Parent.ProjectItems)
            {
                ...
                {
                    if (IsCustomizationProject(pi.Object as Project))
                    {
                        return true;
                    }
                }
        ...
        public static ProjectItems GetProjectItemsByPath(ProjectItems projectItems, string destinationFolder)
        {
            ...
            if ((destinationFolder != String.Empty) && (destinationFolder != null))
            {
                ...
                else
                {
                    ...
                    {
                        projectpath = GetFullPathOfProjectItem(projectItems.Parent as Project);
                    }
                    else if (projectItems.Parent is ProjectItem)
                    {
                        projectpath = GetFullPathOfProjectItem(projectItems.Parent as ProjectItem);
                    }
        ...
        internal static void EnsureCheckout(DTE dte, ProjectItem item)
        {
            try
            {
                string itemname1 = GetFullPathOfProjectItem(item);
                EnsureCheckout(dte, itemname1);
            }
            catch { }
        }
        ...
        internal static void EnsureCheckout(DTE dte, Project project)
        {
            try
            {
                string projectname = GetFullPathOfProjectItem(project);
                EnsureCheckout(dte, project.FullName);
            }
            catch
            {
            }
        ...
        public static string GetResourceValue(string title)
        {
            ...
            if (resfilename != "")
            {
                ...
                string resfile1 = GetSharePointHive() + @"\Resources\" + resfilename + ".resx";
                string resfile2 = GetSharePointHive() + @"\Resources\" + resfilename + ".en-US.resx";
        ...
        internal static bool IsFeatureScope(ProjectItem featureFolder, string checkForScopes)
        {
            ...
            try
            {
                try
                {
                    ...
                    ISharePointProjectFeature sharePointFeature = projectService.Convert<ProjectItem, ISharePointProjectFeature>(featureFolder);
                ...
                ProjectItem featureXML = GetProjectItemByName(((ProjectItem)featureFolder).ProjectItems, "feature.xml");
                if (featureXML != null)
                {
                    string featurepath = GetFullPathOfProjectItem(featureXML);
        ...
        {
            LogMessage(dte, dte, "*** Recycling all applicatin pools ***");
            using (DirectoryEntry appPoolsEntry = new DirectoryEntry(string.Format("IIS://{0}/W3SVC/AppPools", "localhost")))
            {
                foreach (DirectoryEntry AppPool in appPoolsEntry.Children)
                {

                    LogMessage(dte, dte, "Recycle application pool " + AppPool.Name + "...");
        ...
        {
            LogMessage(dte, dte, "Recycling app pool '" + appPoolName + "'");
            using (DirectoryEntry appPoolsEntry = new DirectoryEntry(string.Format("IIS://{0}/W3SVC/AppPools", "localhost")))
            {
                foreach (DirectoryEntry AppPool in appPoolsEntry.Children)
                {
                    if (appPoolName.Equals(AppPool.Name, StringComparison.OrdinalIgnoreCase))
                    {
                        LogMessage(dte, dte, "AppPool " + appPoolName + " found.");
                        AppPool.Invoke("Recycle", null);
                        return;
                    }
                }
            }
            LogMessage(dte, dte, "AppPool " + appPoolName + " not found.");
        }
        ...
        internal static ProjectItem AddFromFileCopy(ProjectItem projectItem, string filename)
        {
            ...
            CheckSharePointReferences(pitem);
        ...
        internal static string GetPathOfProjectItems(ProjectItems projectItems)
        {
            ...
            {
                return GetFullPathOfProjectItem(projectItems.Parent as ProjectItem);
            }
            else
            {
                //return path to the project
                return GetFullPathOfProjectItem(projectItems.ContainingProject);
            }
        }
            ...
            try
            {
                ...
                if (existingItem != null)
                {
                    ...
                    if (overwrite || projectItems.DTE.SuppressUI)
                    {
                        ...
                        string pathToExistingItem = GetFullPathOfProjectItem(existingItem);
                        EnsureCheckout(projectItems.DTE, pathToExistingItem);
                        File.Copy(template, pathToExistingItem, true);
                        LogMessage(projectItems.DTE, projectItems.DTE, pathToExistingItem + ": Contents updated");
                    }
                    else
                    {
                ...
                if (File.Exists(pathToExistingFile))
                {
                    ...
                    EnsureCheckout(projectItems.DTE, pathToExistingFile);
                    File.Copy(template, pathToExistingFile, true);
                    return AddFromFile(projectItems, pathToExistingFile);
                }
...
//                Helpers.LogMessage(projectItems.DTE, projectItems.DTE, Helpers.GetFullPathOfProjectItem(pitem) + ": File Added");
            ...
            {
                LogMessage(projectItems.DTE, projectItems.DTE, GetFullPathOfProjectItem(pitem) + ": Already exists");
            }
                    ...
                    if (foundnodes.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            if (featurefolder != null)
                            {
                                ...
                                {
                                    string respath = GetFullPathOfProjectItem(resourceitem);
        ...
        internal static string GetApplicationName(DTE dte)
        {
            ...
            appname = Regex.Replace(appname, @"[^\w\.@-]", string.Empty);
            return appname;
        }
        ...
        internal static string GetSolutionId(DTE service)
        {
            ...
            else
            {
                try
                {
                    ...
                    string projectDirectory = GetProjectFolder(project);
        ...
        internal static bool IsTargetInSTSADMCustomizationProject(object target)
        {
            try
            {
                ...
                if (checkProject != null)
                {
                    ...
                    string stsadmConfigFilesDir = GetFullPathOfProjectItem(checkProject) + "\\" + GetSharePointVersion(checkProject.DTE) + "\\CONFIG";
        ...
        internal static bool IsTargetInCustomizationProject(object target)
        {
            ...
            if (checkProject != null)
            {
                if (IsCustomizationProject(checkProject))
                {
                    return true;
                }
        ...
        internal static void GetResourcesInProject(Project project, Dictionary<string, CustomResXResourceReader> dict)
        {
            try
            {
                string projectDir = Directory.GetParent(GetFullPathOfProjectItem(project)).FullName;
        ...
        internal static void GetAllFeaturesInVSTemplate(List<NameValueItem> list, Project project)
        {
            ...
            try
            {
                featuresFolder = GetProjectItemByName(project.ProjectItems, "Features");
            }
            catch (Exception)
            {
            }
            ...
            if (featuresFolder != null)
            {
                foreach (ProjectItem feature in featuresFolder.ProjectItems)
                {
                    try
                    {
                        string scope = "Web";
                        string id = "";
                        ProjectItem featureXML = GetProjectItemByName(feature.ProjectItems, feature.Name + ".feature");
                        if (featureXML != null)
                        {
                            string featurepath = GetFullPathOfProjectItem(featureXML);
        ...
        private static void GetAllFeaturesInHIVETemplate(List<NameValueItem> list, Project project)
        {
            ...
            if (featuresFolder != null)
            {
                foreach (ProjectItem feature in featuresFolder.ProjectItems)
                {
                    string scope = "";
                    string id = "";
                    ProjectItem featureXML = GetProjectItemByName(feature.ProjectItems, "feature.xml");
                    if (featureXML != null)
                    {
                        string featurepath = GetFullPathOfProjectItem(featureXML);
        ...
        {
            string version = GetApplicationConfigValue(service, "SharePointVersion", "15");
        ...
        public static XmlDocument GetApplicationConfigFile(DTE dte, bool createIfNotExists)
        {
            ...
            try
            {
                if (!File.Exists(toPath))
                {
                    ...
                    LogMessage(dte, dte, "ApplicationConfiguration.xml not found. Creating file");
        ...
        public static ProjectItem GetFirstFeatureWithScope(DTE dte, Project project, string featureScope)
        {
            ...
            if (Helpers2.IsSharePointVSTemplate(dte, project))
            {
                ...
                foreach (ISharePointProjectFeature existingFeature in sharePointProject.Features)
                {
                    ...
                    if ((featureScope == "Site") && (existingFeature.Model.Scope == Microsoft.VisualStudio.SharePoint.Features.FeatureScope.Site))
                    {
                        ...
                        return projectService.Convert<ISharePointProjectFeature, ProjectItem>(existingFeature);
                    }
            ...
            {
                string sharePointVersion = GetSharePointVersion(dte);
        ...
        {
            string commonProgramFolder = Environment.GetEnvironmentVariable("CommonProgramW6432");
            if (string.IsNullOrEmpty(commonProgramFolder))
            {
                commonProgramFolder = Environment.GetFolderPath(Environment.SpecialFolder.CommonProgramFiles);
            }
        ...
        {
            string programFolder32 = Environment.GetEnvironmentVariable("ProgramW6432");
            if (string.IsNullOrEmpty(programFolder32))
            {
                programFolder32 = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
            }
        ...
        {
            string programFolder = Environment.GetEnvironmentVariable("ProgramW6432");
            if (string.IsNullOrEmpty(programFolder))
            {
                programFolder = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles);
            }
        ...
        public static void DebuggerAttachToW3WP(DTE dte, string basePath)
        {
            try
            {
                ...
                {
                    LogMessage(dte, dte, "No Project selected, starting the StartupProject");
                ...
                if (debuggingWebApp == "")
                {
                    LogMessage(dte, dte, "Warning: Could not find parameter 'DebuggingWebApp' in file Application.config");
                    return;
                }
                ...
                {
                    LogMessage(dte, dte, "Building project " + currentProject.Name);
                    ...
                    if (dte.Solution.SolutionBuild.LastBuildInfo != 0)
                    {
                        ...
                        LogMessage(dte, dte, "No successfull build. Cancelling...");
                        return;
                    }
                    ...
                    {
                        //full deploy
                        LogMessage(dte, dte, "Starting project " + currentProject.Name);
                ...
                if (appPoolOfWebApp == "")
                {
                    LogMessage(dte, dte, "Warning: Could not retrieve web application in IIS");
                    return;
                }

                RecycleAppPool(dte, appPoolOfWebApp);
                ...
                for (int i = 0; i < 20; i++)
                {
                    LogMessage(dte, dte, "Waiting for recycle of application pool " + appPoolOfWebApp);
                    if (IsAppPoolRunning(dte, appPoolOfWebApp))
                    {
                        break;
                    }
                    System.Threading.Thread.Sleep(2000);
                ...
                for (int i = 0; i < 20; i++)
                {
                    LogMessage(dte, dte, "Waiting for restart of SharePoint...");
                    if (PingServer(dte, debuggingWebApp))
                    {
                        break;
                    }
                    System.Threading.Thread.Sleep(2000);
                ...
                string processIdOfWebApp = GetW3WPProcessIdForApplicationPool(dte, appPoolOfWebApp);
                for (int i = 0; i < 20; i++)
                {
                    LogMessage(dte, dte, "Waiting for AppPool restarted...");
                    processIdOfWebApp = GetW3WPProcessIdForApplicationPool(dte, appPoolOfWebApp);
                ...
                LogMessage(dte, dte, "ProcessId for " + appPoolOfWebApp + " is " + processIdOfWebApp);


                //3. Get W3WP Process of ApplicationPool
                if (processIdOfWebApp == "")
                {
                    LogMessage(dte, dte, "Warning: w3wp process not found for application pool " + appPoolOfWebApp);
                    return;
                }
                ...
                foreach (EnvDTE.Process proc in processes)
                {
                    ...
                    {
                        LogMessage(dte, dte, "Successfully found process in VS with process id '" + processIdOfWebApp + "', ready for attaching to process");
                        Trace.TraceInformation("Attaching OK");
                        proc.Attach();
                    }
                }

                OpenWebPage(dte, debuggingWebApp);
            }
            catch (Exception ex)
            {
                LogMessage(dte, dte, ex.Message);
            }
        }
        ...
        internal static string GetAppPoolOfWebApp(DTE dte, string debuggingWebApp)
        {
            ...
            LogMessage(dte, dte, "Retrieving webapplication for " + debuggingWebApp + " (" + debuggingWebAppHost + ":" + debuggingWebAppPort + ")");
        ...
        private static bool PingServer(DTE dte, string deploymentWebapp, int timeOutMilliSeconds)
        {
            ...
            {
                //open the webpage
                LogMessage(dte, dte, "Requesting url '" + deploymentWebapp + "' to force restart of w3wp.exe...");
            ...
            {
                LogMessage(dte, dte, "Requesting url '" + deploymentWebapp + "' returned " + ex.Message);
            }
            return false;
        }
        ...
        {
            LogMessage(dte, dte, "Searching for w3wp process with application pool '" + appPoolOfWebApp + "'");
                ...
                try
                {
                    if (mo["Name"].ToString() == "w3wp.exe")
                    {
                        ...
                        if (apppool == appPoolOfWebApp)
                        {
                            ...
                            LogMessage(dte, dte, "Successfully identified w3wp process, using process id  '" + processIdOfWebApp + "'");
        ...
        {
            LogMessage(dte, dte, "Opening web page " + url);
            Process snProcess = new Process();
...
#if DEBUG
            Log("GaxBaseDir (AssemblyDir): "+gaxBaseDir);
#endif
...
//#endif
        ...
        internal static void AttachToUserCodeHost(DTE dte)
        {
            ...
            if (currentProject == null)
            {
                LogMessage(dte, dte, "No Project selected");
                return;
            }
            AttachToUserCodeHost(currentProject.DTE, currentProject);
        ...
        internal static void AttachToUserCodeHost(DTE dte, Project project)
        {
            ...
            string processIdOfWebApp = "";
            LogMessage(dte, dte, "Searching for SPUCWorkerProcess.exe process (sandboxed solution)");
            ManagementClass MgmtClass = new ManagementClass("Win32_Process");
            foreach (ManagementObject mo in MgmtClass.GetInstances())
            {
                try
                {
                    ...
                    {
                        processIdOfWebApp = mo["ProcessId"].ToString();
                        LogMessage(dte, dte, "Successfully identified SPUCWorkerProcess.exe process, using process id  '" + processIdOfWebApp + "'");
                        break;
                    }
                }
                catch (Exception)
        ...
        internal static void StartNewInstance(DTE dte)
        {
            ...
            if (userResult != DialogResult.Cancel)
            {
                if (userResult == DialogResult.Ignore)
                {
                    ...
                    AttachProjectToW3WP(currentProject.DTE, currentProject, appPoolOfWebApp);
                    OpenWebPage(currentProject.DTE, startupUrl);
                }
                else
                {
                    LogMessage(dte, dte, "Building project " + currentProject.Name);
        ...
        static void startNewProcessQuickDeployWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            if (e.Argument is Project)
            {
                ...
                RecycleAppPool(project.DTE, appPoolOfWebApp);

                WaitForAppPool(project.DTE, appPoolOfWebApp);
                RefreshServer(project.DTE, debuggingWebApp);
                AttachProjectToW3WP(project.DTE, project, appPoolOfWebApp);
                OpenWebPage(project.DTE, startupUrl);
            }
        }
        ...
        {
            string processIdOfWebApp = "";
            if (GetIsSandboxedSolution(project))
            {
        ...
        private static void AttachToVSProcess(DTE dte, string processIdOfWebApp)
        {
            ...
            if (processIdOfWebApp == "")
            {
                LogMessage(dte, dte, "Warning: Process not found");
                return;
            }
        ...
        private static void WaitForAppPool(DTE dte, string appPool)
        {
            for (int i = 0; i < 20; i++)
            {
                LogMessage(dte, dte, "Waiting for recycle of application pool " + appPool);
                if (IsAppPoolRunning(dte, appPool))
                {
                    break;
                }
                System.Threading.Thread.Sleep(2000);
        ...
        {
            string itemname = GetFullPathOfProjectItem(projectItem);
            if (service.SourceControl.IsItemUnderSCC(itemname))
            {
                ...
                LogMessage(service, service, "Removed write protection from " + itemname);
                foreach (ProjectItem childItem in projectItem.ProjectItems)
                {
                    string childFilename = GetFullPathOfProjectItem(childItem);
                    File.SetAttributes(childFilename, FileAttributes.Normal);
                    LogMessage(service, service, "Removed write protection from " + childFilename);
                }
            }
        }
